 ...
 <div id="box">
  ...
  <div id="left" class="fl" style="margin-top:8px;">
    <div class="left_content1">
      <div id="printarea">
          <?php if(is_array($data)): foreach($data as $key=>$in): ?><h1 class="infotitle"><?php echo ($in["a_title"]); ?></h1>
...
<script type="text/javascript" src="http://bbs.itcast.cn/api.php?mod=js&bid=94"></script>
      ...
      <div class="pre">
	               上一篇：<a target="_blank" href="/news/20141208/14061592998.shtml"><?php echo ($in["a_title"]); ?></a>
       	  </div>
      <div class="next">
...
<!-- Baidu share END 需在页面底部添加baidushare.js -->	  
	  ...
	  <!-- 评论列表 -->
              <div id="dis">
	  <!-- 评论列表 -->
            <?php if(is_array($info)): foreach($info as $key=>$it): ?><h3 class="reviewtitle"><span class="fl"><?php echo ($it["a_region"]); ?> </span><span class="fr"><?php echo date('Y-m-d H:i',$it['start_time']) ?> 发表</span></h3>
        <div class="reviewneirong"><?php echo ($it["a_comment"]); ?>   </div><?php endforeach; endif; ?>
          </div>
            <!-- 评论列表 -->
        <div class="febgye"><?php echo $page?></div>
	  ...
	  <form name="comment" style="padding:0;margin:0px;">
        ...
        <div class="clear"></div>
        <div class="fabiao">
	      <span class="fl">请输入验证码：<input name="validatecode" id="code" type="text" style="width:80px;" />  <img width="100" id="captcha-container" class="left15" alt="验证码" src="/Index/Jyjb/verify_c" title="点击刷新"> </span><span class="fr"><input type='button' value="提 交" id="savebutton" onclick="add()"/></span>
	    </div>
	  </form>
      ...
      <input type="hidden" id="a_id" value="<?php echo $data[0]['aid']?>">
      <script>
    ...
    $.getScript('http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js&ip='+ip, function(_result){ 
        if (remote_ip_info.ret == '1'){  
             p=remote_ip_info.province+'省'+remote_ip_info.city+'市  网通网友 ip：'+ip+ ' 游客';  
            $("#b").val(p);
        }     
           })
          ...
          function add(){
              ...
              // return false;
              $.ajax({
                  url:"/Index/Jyjb/article_comment",
                  data:{"content":content,"code":code,"region":b,"a_id":a_id},
                  dataType:"json",
                  type:"get",
                  success:function(res){
                      if(res==0){
                          ...
                          str="";
                          for(a in res){
                              str+="<h3 class=\"reviewtitle\"><span class=\"fl\">"+res[a]["a_region"]+"<\/span><span class=\"fr\"> "+res[a]["start_time"]+" 发表<\/span><\/h3><div class=\"reviewneirong\">"+res[a]["a_comment"]+"<\/div>";
                          }
                          $("#dis").html(str);
                           $("#content").val("");
                          $("#code").val("");
