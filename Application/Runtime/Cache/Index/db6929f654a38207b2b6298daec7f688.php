...
<!-- Baidu share END 需在页面底部添加baidushare.js -->	  
	  ...
	  <div class="clear"></div>
          ...
          <div id="dis">
	  <!-- 评论列表 -->
            <?php if(is_array($info)): foreach($info as $key=>$it): ?><h3 class="reviewtitle"><span class="fl"><?php echo ($it["c_region"]); ?> </span><span class="fr"><?php echo date('Y-m-d H:i',$it['start_time']) ?> 发表</span></h3>
        <div class="reviewneirong"><?php echo ($it["c_comment"]); ?>   </div><?php endforeach; endif; ?>
          </div>
            <!-- 评论列表 -->
	  ...
	  <form name="comment" style="padding:0;margin:0px;">
        ...
        <div class="clear"></div>
        <div class="fabiao">
	      <span class="fl">请输入验证码：<input name="validatecode" id="code" type="text" style="width:80px;" />  <img width="100" id="captcha-container" class="left15" alt="验证码" src="/Index/Question/verify_c" title="点击刷新"> </span><span class="fr"><input type='button' value="提 交" id="savebutton" onclick="add()"/></span>
	    </div>
	  </form>
                    <?php
 } ?>
      ...
      <input type="hidden" id="a_id" value="<?php echo $data[0]['p_id']?>">
      <script>
              ...
              $.ajax({
                  ...
                  success:function(res){
                      if(res==0){
                          ...
                          str="";
                          for(a in res){
                              str+="<h3 class=\"reviewtitle\"><span class=\"fl\">"+res[a]["c_region"]+"<\/span><span class=\"fr\"> "+res[a]["start_time"]+" 发表<\/span><\/h3><div class=\"reviewneirong\">"+res[a]["c_comment"]+"<\/div>";
                          }
                          $("#dis").html(str);
                           $("#content").val("");
                          $("#code").val("");
